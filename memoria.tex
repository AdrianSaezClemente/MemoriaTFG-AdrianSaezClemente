...
\begin{figure}
  \centering
  \doublebox{\includegraphics[width=17cm, keepaspectratio]{imagenes/PersonalizarDiseno}}
...
%   Cada uno de ellos tiene asociado un bot\'on que si se pulsa se realizar\'a una petici\'on \texttt{AJAX}
%   de tipo \texttt{GET} que ser\'a procesada por la vista \texttt{eliminardiseno}. Esta vista identifica la petici\'on como \texttt{GET} y mediante el id del dise\~no pasado como dato en la 
%   petici\'on, filtra en la tabla \texttt{Diseno} y borra el registro. Posteriormente, env\'ia una respuesta \texttt{HTTP} redireccionando a la misma p\'agina de dise\~nos de la aplicaci\'on, 
%   por lo que la vista renderizar\'a la plantilla \texttt{diseno.html}.
...
\begin{itemize}
  ...
  \item {\bfseries \textit{Pulsar en bot\'on de entrada le\'ida o en link de t\'itulo de la entrada}}: A la derecha de cada id de entrada, 
  hay un bot\'on con un icono que simboliza un ojo tachado, tal y como se puede ver en la figura~\ref{fig:LecturaEntrada}. Tanto si se presiona ese bot\'on o
  si pulsa el link del t\'itulo de la entrada, se llamar\'a a la funci\'on \textit{JavaScript} \texttt{EntradaLeida} mediante el evento \textit{onClick}. 
  Esta funci\'on se define en el fichero \texttt{index.js}. 
  
  Dicha funci\'on env\'ia una petici\'on \textit{AJAX} de tipo \texttt{GET} a su correspondiente vista mediante su recurso o URL. La petici\'on es enviada 
  desde el fichero \texttt{index.js} usando \textit{jQuery} e incluye como datos el Id de la asignatura, el Id del usuario, el Id de la entrada y un
  booleano (indica si la entrada ha sido visitada o no).
  
  La vista \texttt{entradaleida} crear\'a un objeto en el modelo \texttt{Extra} con los par\'ametros pasados en la solicitud. Guarda el objeto y modifica 
  la entrada del modelo \texttt{Entrada} en la que se efectu\'o el click del bot\'on ``Leído''. En el objeto \texttt{Entrada} se cambia el campo \textit{visitantes} 
  (en caso de que el booleano sea true, es decir, se haya visitado el post al acceder mediante el t\'itulo de la entrada) y el campo 
  \textit{visitas} (indica el total de visitas que ha recibido esa entrada). Por \'ultimo, la vista modifica el objeto \texttt{Extra} 
  buscando el registro correspondiente a esa entrada y esa asignatura, comprueba si el usuario se encuentra en la lista de ids del campo \texttt{Leido} y si no se encuentra, 
  se a\~nade el id del usuario.
  
  La funci\'on \textit{AJAX} recibe una respuesta con c\'odigo \textit{HTML}. Mediante la funci\'on \textit{jQuery} \texttt{.html} en la entrada del documento \textit{HTML} 
  se crea un bot\'on deshabilitado y se modifica la cantidad total de visitas que ha recibido.
  
  \begin{figure}
    \centering
    \subfigure[Entrada le\'ida.]{\Ovalbox{\includegraphics[width=7cm]{imagenes/LecturaEntradaLeida}}}
    \subfigure[Entrada no le\'ida.]{\Ovalbox{\includegraphics[width=7cm]{imagenes/LecturaEntradaNoLeida}}}
    \caption{\textit{Lectura de una entrada.}}
    \label{fig:LecturaEntrada}
  \end{figure}
  
  \item {\bfseries \textit{Mostrar visitantes}}: Al pulsar sobre el n\'umero subrayado de visitas totales de una entrada (figura~\ref{fig:mostrarvisitantes}), se llamar\'a a la 
  funci\'on \textit{JavaScript} \texttt{MostrarVisitasUsuarios} mediante el evento \textit{click} de \textit{jQuery}. Esta funci\'on se define en el fichero \texttt{index.js}. 
  
  Dicha funci\'on env\'ia una petici\'on \textit{AJAX} de tipo \texttt{GET} a su correspondiente vista mediante su recurso o URL. La petici\'on es enviada 
  desde el fichero \texttt{index.js} usando \textit{jQuery} e incluye como dato el Id de la entrada.
  
  La vista \texttt{mostrarvisitasusuarios} env\'ia como respuesta a la funci\'on \textit{AJAX} una lista de los nombres de los visitantes almacenados en el campo \texttt{visitantes} 
  de la entrada. Mediante la funci\'on \textit{jQuery} \texttt{.html} en la entrada del documento \textit{HTML} se crea una tabla con los nombres de dichos visitantes y se visualizar\'an 
  en un popup emergente. En caso de que la lista est\'e vac\'ia se mostrar\'a un mensaje ``Esta entrada no tiene visitantes''.
  
  Si se vuelve a pulsar sobre el n\'umero subrayado de visitas totales de dicha entrada, el popup se cerrar\'a.
  
  \begin{figure}
    \centering
    \subfigure[Lista de visitantes no vac\'ia.]{\Ovalbox{\includegraphics[width=7cm]{imagenes/VisitantesLleno}}}
    \subfigure[Lista de visitantes vac\'ia.]{\Ovalbox{\includegraphics[width=7cm]{imagenes/VisitantesVacio}}}
    \caption{\textit{Visitantes de una entrada.}}
    \label{fig:mostrarvisitantes}
  \end{figure}
  ...
  \begin{figure}
    \centering
    \subfigure[Entrada valorada en hilo de tutor.]{\Ovalbox{\includegraphics[width=10cm]{imagenes/EntradaTutorConValoracion}}}
    \subfigure[Entrada sin valorar en hilo de tutor.]{\Ovalbox{\includegraphics[width=10cm]{imagenes/EntradaTutorSinValoracion}}}
  ...
  \begin{figure}
    \centering
    \Ovalbox{\includegraphics[width=17cm, keepaspectratio]{imagenes/EliminarDiseno}}
...
\begin{figure}[htbp] 
  \centering
  \doublebox{\includegraphics[width=17cm, keepaspectratio]{imagenes/ListaPersonalizarDiseno}}
  \caption{}
...
\begin{figure}
  \centering
  \subfigure[Sin dise\~no.]{\Ovalbox{\includegraphics[width=7cm]{imagenes/PresentacionAlumno}}}
  \subfigure[Con dise\~no.]{\Ovalbox{\includegraphics[width=7cm]{imagenes/PresentacionAlumnoConDiseno}}}
...
\begin{figure}
  \centering
  \subfigure[P\'agina completa de personalizaci\'on.]{\Ovalbox{\includegraphics[width=7cm]{imagenes/PersonalizarDiseno}}}
  \subfigure[Secci\'on para eliminar dise\~nos.]{\Ovalbox{\includegraphics[width=7cm]{imagenes/EliminarDiseno}}}
  \caption{\textit{Gestionar dise\~nos personalizados.}}
...
\begin{itemize}
  ...
  \begin{figure}[htbp] 
    \centering
    \Ovalbox{\includegraphics[width=17cm, keepaspectratio]{imagenes/LecturaEntradaLeidayNoLeida}}
